...
-- 6. Habilitar Realtime com segurança (Tratando erro se já estiver habilitado)
DO $$
DECLARE
    t text;
BEGIN
    FOR t IN SELECT unnest(ARRAY['patients', 'consultations', 'inventory', 'suppliers']) LOOP
        BEGIN
            EXECUTE format('ALTER PUBLICATION supabase_realtime ADD TABLE public.%I;', t);
        EXCEPTION WHEN duplicate_object THEN
            -- IGNORA: A tabela já está na publicação do Realtime
            NULL;
        END;
    END LOOP;
END;
