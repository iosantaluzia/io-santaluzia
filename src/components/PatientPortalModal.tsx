  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const {
        data,
        error
      } = await signInWithUsername(formData.email, formData.password);
      if (error) {
        ...
        <form onSubmit={isLogin ? handleLogin : handleRegister} className="space-y-4">
          {!isLogin && <>
              <div>
                ...
                <Input id="name" name="name" type="text" value={formData.name} onChange={handleInputChange} required={!isLogin} />
              </div>
              <div>
                <Label htmlFor="cpf">CPF</Label>
                <Input id="cpf" name="cpf" type="text" value={formData.cpf} onChange={handleInputChange} required={!isLogin} placeholder="000.000.000-00" />
              </div>
            </>}
          ...
          <div className="text-center">
            <button type="button" onClick={toggleMode} className="text-sm text-medical-secondary hover:text-medical-primary transition-colors">
              {isLogin ? 'Não tem conta? Cadastre-se aqui' : 'Já tem conta? Faça login aqui'}
            </button>
          </div>

          {isLogin && <div className="text-center">
              <button type="button" className="text-sm text-gray-500 hover:text-medical-primary transition-colors">
                Esqueceu sua senha?
              </button>
            </div>}
        </form>
